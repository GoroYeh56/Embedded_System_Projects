...
//QueueHandle_t mini_Queue_Handle = 0;
 TaskHandle_t Map;
...
void setup() {
  ...
  Succeed_Queue_Handle = xQueueCreate(3, sizeof(int));
//  mini_Queue_Handle = xQueueCreate(1,sizeof(int));
  ...
  xTaskCreate(DisplayTask, (const portCHAR *) "Task2",120, NULL, 1, NULL);//Task2 : Display.
  xTaskCreate(mapTask, (const portCHAR *) "Task3", 64, NULL, 1, &Map);//Task3 : Create initial map.    
...
//        vTaskDelay(500/portTICK_PERIOD_MS);//delay 3 sec     
        ...
        if(xQueueReceive(Succeed_Queue_Handle,&succeed,1000)){ 
         ...
         Serial.println(F("Failed to receive from queue")); 
        if(xQueueReceive(Succeed_Queue_Handle,&mineX,1000)){ 
        }
        else 
         Serial.println(F("Failed to receive from queue"));         
        if(xQueueReceive(Succeed_Queue_Handle,&mineY,1000)){ 
        }
        else 
         Serial.println(F("Failed to receive from queue")); 
...
//          Serial.println("Get mine x");
////          Serial.println(use_remover);   
//       }
//        else 
//         Serial.println("Failed to receive from- queue");    
//        if(xQueueReceive(Map_Queue_Handle,&mine_y,1000)){
//          Serial.println("Get mine y");
////          Serial.println(use_remover);   
//       }
//        else 
//         Serial.println("Failed to receive from queue");
...
//            last_x = last_y = 0;
            ...
            if(use_remover == 1){
                ...
                if(col== mine_x && row == mine_y){
                    ...
                    if(abs(mine_x - col) < 3 ){ //distance smaller than 3.
...
//                      last_y = row;
        ...
        if(!xQueueSend(Succeed_Queue_Handle, &succeed, 1000)) //Send to control task.
           Serial.println("Failed to send to queue");
        if(!xQueueSend(Succeed_Queue_Handle, &mine_x, 1000)) //Send to control task.
           Serial.println("Failed to send to queue");
        if(!xQueueSend(Succeed_Queue_Handle, &mine_y, 1000)) //Send to control task.
           Serial.println("Failed to send to queue");                      
